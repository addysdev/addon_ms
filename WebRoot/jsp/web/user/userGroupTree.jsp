...
String PopName = (String)model.get("PopName");
%>
...
$(function(){
	$("#tree<%=FormName%>1").dynatree({
		...
		onActivate : function(node){
			...
			$('#<%=FormName%> #GroupName').val(node.data.title);
			$('#<%=FormName%> #GroupID').val(node.data.key);
			$('#<%=PopName%>').dialog('close');
		}
		
	});

	var rootNode = $("#tree<%=FormName%>1").dynatree("getRoot");
	<%
	if(grouplist.size() > 0){
		int i = 0;
		String cateid="";
...
<body>
	<form action="">
			<div id="tree<%=FormName%>1" style="margin-top:10px; margin-left:-8px; width: 250px; width: 248px\9; width: 250px\0/IE8+9; height: 215px; height: 203px\9; height: 213px\0/IE8+9;" name="selNodes"></div>
