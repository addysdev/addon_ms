...
function goSearch() {
	...
	obj.action = "<%=request.getContextPath()%>/H_Order.do?cmd=targetPageList";
...
function goTargetDetail(companycode,groupid,groupname){
	$('[name=targetDetailPageList]').dialog({
		...
		open:function(){
			...
			$(this).load('<%=request.getContextPath()%>/H_Order.do?cmd=targetDetailPageList',{
...
<body onLoad="" class="body_bgAll">
	<form method="post" name=TargetForm action="<%=request.getContextPath()%>/H_Order.do?cmd=targetPageList">
			...
			<div class="tbl_type_out">
				...
				<!-- 확인 미확인 구분 볼드처리 시작-->	
                       <tr name="tdonevent" id="tdonevent" style="background-color:<%= rowClass%>;" onmouseover="fnLayerover(<%=i%>);" onmouseout="fnLayerout(<%=i%>,'<%=rowClass%>');"   >                              
		                	<td style="cursor: pointer" onclick="goTargetDetail('<%=ds.getString("CompanyCode")%>','<%=ds.getString("GroupID")%>','<%=ds.getString("GroupName") %>');"><%=ds.getString("GroupID")%></td>
...
<div name="targetDetailPageList" title="발주 대상 상세 품목"></div>
</body>
</html>
