...
function goSaveExcel(){
	...
	var frm=document.AddressRegistForm; //form객체세팅
...
<div id="wrap">
  ...
  <div id="contents" class="userExcelup">
    ...
    <fieldset>
      ...
      <form name="AddressRegistForm" method="post"  action="<%= request.getContextPath() %>/H_Address.do?cmd=AddressExcelImport" enctype="multipart/form-data">
        <span>업로드 할 <em class="bold">엑셀파일</em></span><input type="file"  name="addressFile" />
      </form>
    </fieldset>
	</div>
    <!-- //form_area -->
    ...
    <div class="caution_area">
      ...
      <ul>
        ...
        <li class="cmt">엑셀파일 업로드 양식을 다운로드 합니다. <a href="<%= request.getContextPath() %>/fileDownServlet?rFileName=AddressUploadFormat.xls&sFileName=AddressUploadFormat.xls&filePath=/down"><strong class="blueTxt">[양식다운로드]</strong></a>
          <br />
     <span>(다운받으신 양식의 첫번째 타이틀에 맞춰 내주소록 정보를 저장합니다.)</span> </li>
        <li>팩스번호 입력시 팩스번호가 중복 될 경우 등록실패 됩니다.</li>
  ...
  <!-- button -->
  <div class="ly_foot">
    <a href="javascript:goSaveExcel()"><img src="<%=request.getContextPath()%>/images/popup/btn_add.gif" title="등록" /></a> <a href="javascript:goClosePop()"><img src="<%=request.getContextPath()%>/images/popup/btn_cancel.gif" title="취소" /></a>
  </div>
  <!-- //button -->
